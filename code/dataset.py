...
class CSC_Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, path, config, subset):
        ...
        if self.subset !=  "train":
            self.data = [_ for _ in jsonlines.open(self.path, 'r')]

    def __getitem__(self, idx):
        data = eval(linecache.getline(self.path, idx + 1))
        item = {}
        ...
        if self.subset != 'test':
            item['trg_ids'] = torch.LongTensor(data['trg_ids'])
            item['sequence_cls'] = torch.LongTensor(data['sequence_cls'])
            #item['token_cls'] = torch.LongTensor(data['token_cls'])
            item['weights'] = torch.LongTensor([1. if data['trg_ids'][k] == c else self.config.weight for k, c in enumerate(item['input_ids'])])
        return item

    def __len__(self):
        return int(open(self.path.replace("jsonl", "txt"), "r").read().strip())
    ...
    def __call__(self, batch):
        ...
        for item in batch:
            ...
            if not is_test:
                ...
                #item['token_cls'] = self.pad(item['token_cls'], -1, max_len)
                item['weights'] = self.pad(item['weights'], 0., max_len)
